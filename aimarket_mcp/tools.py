...
SANDBOX_VISITOR = os.environ.get("AIMARKET_SANDBOX_VISITOR") or f"mcp-{uuid.uuid4().hex[:12]}"
...
async def market_search(args: Dict[str, Any]) -> str:
    ...
    for m in matches:
        lines.append(
            ...
            f"  product_id={m.get('product_id')}  trust={m.get('trust_score')}"
            f"  {(m.get('name') or '')[:60]}"
        )
    lines.append("\nCall market_invoke with capability_id + product_id to run one.")
    return sanitize_tool_output("\n".join(lines))
